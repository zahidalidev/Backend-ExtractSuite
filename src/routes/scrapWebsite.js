...
const { websiteScrappingService } = require('../services/scraping/web')
...
router.post('/scrapWebsite', async (req, res) => {
  ...
  try {
    ...
    for (const link of linkArray) {
      const scrapedData = await websiteScrappingService({ link, domains, extractOptions }, false)
      results.push({
        link,
        data: `Scraped data for ${link}`,
        timestamp: new Date()
      })
    }
